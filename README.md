<div align="center">

# 👋 Hi there, I'm Mohamed

### Builder • Developer • Storyteller

I turn raw ideas into fully functional MVPs — complete with clean UI, backend architecture, and real user-ready systems.

Currently building **[Deen n' Dunya](https://deenndunya.org)**, **Sanad+**, and a suite of productivity tools under **[MK Builds](https://mkbuilds.dev)**

*"Build with purpose, ship quickly, and constantly refine using data, design, and first-principles thinking."*

---

### 🔥 390 Contributions in the Last Year

![GitHub Contribution Chart](https://ghchart.rshah.org/667eea/mkbuilds4)

---

</div>

## 💻 Tech Stack

**Mobile & Frontend**  
`Flutter` • `Dart` • `iOS/Android` • `Figma`

**Backend & Database**  
`Supabase` • `PostgreSQL` • `Firebase` • `Firestore` • `Edge Functions`

**Tools & Services**  
`Cursor` • `VS Code` • `Vercel` • `Notion` • `Mixpanel` • `OneSignal` • `Twilio`

**Specialized in:** Mobile-first habit systems • AI-assisted experiences • Real-time tracking • Gamification • Community platforms

---

## 🚀 Featured Projects

### 🌙 [Deen n' Dunya](https://deenndunya.org)
A personal-growth system designed to help people build consistency in faith and life through streaks, daily habits, tokens, and long-term challenges. Features a full token economy, challenges system, shop, streak logic, and cross-platform architecture.

**→ [Visit Deen n' Dunya](https://deenndunya.org)**

---

### 🕌 Sanad+
A spiritual support companion focused on Salah — reminders, reflections, and khushoo-building tools wrapped in a minimal and modern design.

---

### 📚 [Manara](https://manara.mkbuilds.dev)
An education and CRM platform built for local programs to manage attendance, communication, teacher tools, finances, and community operations in one place.

**→ [Visit Manara](https://manara.mkbuilds.dev)**

---

## 🌱 Currently Learning

**Exploring Now**
- Deeper Islamic studies (Salah, khushoo)
- Advanced backend patterns & scalable infrastructure
- Platform-level growth strategies

**What's Next**
- AI coaching inside apps
- Intelligent user-insight systems
- Mini-app ecosystem inside DnD
- On-device AI & agent workflows
- Apple's Mini App Partner Program

---

## 💡 Beyond Code

I'm passionate about helping people grow — spiritually, mentally, and personally — through tools that are simple, modern, and genuinely useful.

I balance building apps with community organizing, personal development, branding, design, and youth mentorship.

**Fun fact:** I've built entire apps, systems, and brands solo — while running community programs and managing academic commitments.

---

## 🤝 Let's Connect

I'm open to **selective collaborations**, especially projects that combine tech, personal growth, and community impact.

I love conversations about: **App building** • **Entrepreneurship** • **Personal growth** • **Islamic education** • **Community systems**

Also open to **mentoring youth and beginners in tech**.

<div align="center">

[![Website](https://img.shields.io/badge/🌐_Portfolio-mkbuilds.dev-667eea?style=for-the-badge)](https://mkbuilds.dev)
[![Instagram](https://img.shields.io/badge/📸_Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white)](https://instagram.com/YOUR_HANDLE)
[![Email](https://img.shields.io/badge/✉️_Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:YOUR_EMAIL)

![Profile Views](https://komarev.com/ghpvc/?username=mkbuilds4&color=667eea&style=for-the-badge)

</div>
